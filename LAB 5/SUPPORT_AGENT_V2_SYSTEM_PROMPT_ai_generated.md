...
| 429 backoff schedule            | `2 ** attempt` seconds — 2s, 4s, 8s (logged/reported value; wall-clock scaled, see sleep-scale row) |
| 500 retry delay                 | `2` seconds between attempts; fallback after attempt 3 (wall-clock scaled)    |
| 529 retry delay                 | `3` seconds; fallback after attempt 3 (wall-clock scaled)                     |
| Backoff sleep-scale knob        | `SLEEP_SCALE = float(os.getenv("STRESS_SLEEP_SCALE", "1.0"))` in `agent.py`; every backoff `time.sleep(x)` runs as `time.sleep(x * SLEEP_SCALE)` via a `_backoff_sleep(seconds)` helper. Default `1.0` = real-time. The **logged and reported** wait values (2/4/8s, 2s, 3s) are unchanged — only physical wall-clock sleep scales. Lets the full 45-run suite finish in a fraction of the time without altering the documented recovery policy. |
...
| 2 | `support_agent_v2_colab.ipynb`             | self-contained notebook: 14 code cells + markdown docs | the entire harness in one file: markdown overview → install deps → create modular .py files via `%%writefile` → recovery-strategy markdown → set key via parent-kernel `getpass` → run suite → reading-the-report markdown → download the JSON report |
...
#### 8.0 Backoff speed (fast vs. real-time)

The faults are **simulated**, so the retry backoff waits (the
`2 ** attempt`, `2`, and `3` second sleeps) are the single biggest
chunk of wall-clock time on the `api_429`/`api_500`/`api_529`/`chaos`
runs — roughly **8 minutes of the suite is spent asleep**. Because
`agent.py` routes every backoff through `_backoff_sleep` and the
`SLEEP_SCALE` env knob (Section 7.7), this time can be collapsed
**without changing the documented 2/4/8s recovery policy** — the logged
and reported wait values are unaffected.

Render a **structured chooser** per Section 5.0:

- **Option 1 — Fast run (`STRESS_SLEEP_SCALE=0.05`)** `(Recommended)`
  - Description: "Shrinks the simulated backoff sleeps ~20×. Full
    45-run suite finishes in roughly **5–8 minutes** on Sonnet. The
    report still documents the true 2/4/8s recovery schedule."
- **Option 2 — Real-time backoff (`STRESS_SLEEP_SCALE=1.0`)**
  - Description: "Physically waits the full 2/4/8s on every retry.
    Roughly **12–18 minutes** for the full suite. Use for the final
    'official' timing run."

Set the chosen value in the environment that launches the runner.

Before launching:

> Running stress suite. With fast backoff (`STRESS_SLEEP_SCALE=0.05`)
> expect roughly **5–8 minutes** for the full 45-run suite on Sonnet
> (or **12–18 minutes** at real-time backoff), faster for subsets.
> You'll see per-run status lines (`[NN/45] <scenario> × <fault> ...
> PASS | errors=N recovered=N`) as they complete.
...
- **Variant 1 (Python):**
  ...
  3. Set `STRESS_SLEEP_SCALE` in the subprocess environment per the
     Step 8.0 choice (e.g. PowerShell `$env:STRESS_SLEEP_SCALE="0.05"`
     before the call, or pass it in the `env` of the spawned process).
  4. Execute (with `cwd` set to the active folder):
     ```
...
**Imports.** `os`, `json`, `time`, `anthropic`, `typing.Any`,
...
- `MAX_TOKENS = 1024`.
- `SLEEP_SCALE = float(os.getenv("STRESS_SLEEP_SCALE", "1.0"))` — a
  wall-clock multiplier for the simulated backoff waits. Default `1.0`
  preserves real-time backoff. Setting `STRESS_SLEEP_SCALE=0.05` in the
  environment shrinks the suite's physical sleeping (the dominant cost of
  the `api_429`/`api_500`/`api_529`/`chaos` runs) while leaving every
  logged and reported wait value untouched.

**Module-level helper.**

- `_backoff_sleep(seconds)` — calls `time.sleep(seconds * SLEEP_SCALE)`.
  **Every** backoff wait in `chat()` (the 429 `2 ** attempt`, the 500
  `2`, the 529 `3`) must go through this helper, **not** a raw
  `time.sleep`. The `RETRY`/`RECOVER` log lines still name the
  unscaled seconds (e.g. "waiting 8s") so `stress_test_report.json`
  documents the true 2/4/8s recovery policy regardless of `SLEEP_SCALE`.
...
- **Recovery strategies implemented** — a dedicated section (the same
  content required for the Python variant in §7.9 item 7) with four
  subsections, so the "document how the system recovers" requirement is
  satisfied in prose, not only inferable from the report metrics:
  - **API errors** — 429 (`2 ** attempt` backoff: 2s/4s/8s, then
    retry), 500 (retry @2s ×3, then `server_error` fallback), 529 (3s
    delay + resume, then `max_retries` fallback with an `ESC-` id).
  - **Context overflow** — char/4 token estimate, 80% threshold,
    summarize oldest 50% via Claude, preserving order IDs, amounts,
    decisions, and promises to the customer.
  - **Tool failures** — timeout (graceful "try again shortly" result),
    malformed (extract partial fields and proceed honestly), cascade
    (first call times out, downstream tools isolated/blocked).
  - **User experience** — no raw errors or stack traces ever reach the
    customer; partial data is flagged plainly; exhausted retries route
    to a human-escalation path with a reference ID.
...
**The notebook has exactly 14 code cells** in this order. **Markdown
documentation cells are also required** (see §8.3.1); they do **not**
count toward the 14 and do **not** change the code-cell numbering used
throughout this spec — every "cell N" reference means the Nth *code*
cell, regardless of interleaved markdown.
...
#### 8.3.1 Required markdown documentation cells

The notebook is the artifact most readers open first, so the recovery
story must be visible inside it — not only in `README.md`. Add these
**markdown** cells (they do not affect the 14-code-cell count or the
code-cell numbering):

- **A title + overview cell at the very top** (before code cell 0):
  H1 title, the one-paragraph description of the harness, and the
  compact architecture diagram from Section 3 (fenced as a code block).
- **A "Recovery strategies" markdown cell placed immediately before the
  runner cell (code cell 13).** It must contain the same four
  subsections required of the README in §8.2 — API errors (429/500/529
  with their backoff schedules), context overflow (estimate, 80%
  threshold, summarize oldest 50%, facts preserved), tool failures
  (timeout / malformed / cascade handling), and user experience (no raw
  errors, partial data flagged, escalation path with reference ID).
- **A "Reading the report" markdown cell after the runner cell.** It
  names the four key metrics (`errors_injected` vs `recovery_actions`
  = recovery rate, `duration_s` = latency cost, `all_turns_completed`,
  `hard_failures`) and shows one sample report entry.

The recovery-strategy prose must stay in sync with what `agent.py`
actually does; if a fallback message or backoff schedule changes, update
this cell too.
...
- [ ] Cell 13 (`!python run_stress_tests.py`) runs after cell 12 in
      execution order.
- [ ] The notebook contains the markdown documentation cells required
      by §8.3.1: a top-of-notebook title/overview/diagram cell, a
      "Recovery strategies" cell (four subsections) immediately before
      the runner cell, and a "Reading the report" cell after it.
- [ ] The Colab `README.md` contains the "Recovery strategies
      implemented" section required by §8.2 (four subsections covering
      API errors, context overflow, tool failures, user experience).
